									...
									<div class="caption font-dark">
										...
										$type=$this->input->get('type');
											if($this->session->userdata('tasks_view')=="tasks_view"){
											    if($monthly=="true"){
			$main_data=$this->db->get_where("tbl_tasks",array('project_id'=>$id_projects,'task_type'=>$type,'status'=>'2','real_date>='=>date("Y-m-01"),'real_date<='=>date("Y-m-t")))->result();
											    }
											    else {
										$main_data=$this->db->get_where("tbl_tasks",array('project_id'=>$id_projects,'task_type'=>$type,'status'=>'2'))->result();
											    }
											}
											else {
											     if($monthly=="true"){
	$main_data=$this->db->get_where("tbl_tasks",array('project_id'=>$id_projects,'user_id'=>$user_id,'task_type'=>$type,'status'=>'2','real_date>='=>date("Y-m-01"),'real_date<='=>date("Y-m-t")))->result();										         
											     }
											      else {
	$main_data=$this->db->get_where("tbl_tasks",array('project_id'=>$id_projects,'user_id'=>$user_id,'task_type'=>$type,'status'=>'2'))->result();										          
											      }
...
$(document).ready(function(e) {
	...
	$(".waitbutton").click(function(e) {
        ...
        window.location.assign("<?= DIR?>admin/task/monthly_tasks?task_type="+<?=$this->input->get('task_type');?>+"&id_project="+<?=$this->input->get('id_project');?>);
    });
        ...
        window.location.assign("<?= DIR?>admin/task/status_task?task_type="+<?=$this->input->get('task_type');?>+"&task_status=2&id_project="+<?=$this->input->get('id_project');?>);
    });
    	$(".button_new_status").click(function(e) {
        window.location.assign("<?= DIR?>admin/task/status_task?task_type="+<?=$this->input->get('task_type');?>+"&task_status=1&id_project="+<?=$this->input->get('id_project');?>);
    });
