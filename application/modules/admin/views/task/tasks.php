							...
							<div class="portlet light bordered">
								<div class="portlet-title">
									<div class="caption font-dark">
										<i class="icon-settings font-dark"></i><span class="caption-subject bold uppercase"><?= lang('task'); ?> 
										<br>
										...
										$main_data=$this->db->get_where("tbl_tasks",array('project_id'=>$id_projects))->result();
											}
											else {
										$main_data=$this->db->get_where("tbl_tasks",array('project_id'=>$id_projects,'user_id'=>$user_id))->result();
											}
...
<div class="row" style="margin:0px">
					...
					for($i=0; $i<count($type_id); $i++){
					    ...
					    if(count($main_arr)>0){ if($main_arr[$i]>0){$reaming_total_depceil=($main_arr[$i]-$dep_progress);}else {$reaming_total_depceil=($main_arr[$i]-$dep_progress);} }
                           ?>
							<div class="col-md-<?php if(count($type_id)>2){?>4<?php } else {?>6<?php }?> col-xs-12" style="float:left;direction: ltr; padding-right:2px;padding-left:2px;">
							    ...
							    <span style="font-size:10px;font-weight:600;text-transform:capitalize;">
							        All: </span><span style="text-transform:capitalize;color: #d00606;font-size:11px;">(<?= $main_arr[$i]."hrs";?>)</span><?php }?>
							        <?php if($dep_progress!=""){?>
							         <span style="font-size:10px;font-weight:600;text-transform:capitalize;">Used:</span>
							         <span style="text-transform:capitalize;color: #d00606;font-size:11px;">(<?= $dep_progress."hrs";?>)</span>
							         <?php }if($reaming_total_depceil!=""){ ?>
							         <span style="font-size:10px;font-weight:600;text-transform:capitalize;">Remaining:</span>
  ...
  if(count($main_arr)>0){ if($main_arr[$i]>0){$reaming_total_depceil=($main_arr[$i]-$dep_progress);}else {$reaming_total_depceil=($main_arr[$i]-$dep_progress);} }
                           ?>
							<div class="col-md-<?php if(count($type_id)>2){?>4<?php } else {?>6<?php }?> col-xs-12" style="float:left;direction: ltr; padding-right:2px;padding-left:2px;">
								...
								<?php 
							$sub_total_hrs=round(get_sub("user_task_log","total_hrs",array("id_project"=>$id_projects,'status'=>'2'))/60,2);
							 //echo $sub_total_hrs;
								?>
						<div class="row" style="margin:0px;">
								<hr style="margin:5px 0;">
							<h3 style="font-size:13px;color: #e7505a;">
							    <span><i class="fa fa-clock-o" style="margin-right:5px;"></i></span><?= lang("project duration total");?>:
							     ...
							     <?php if($total_hrs_project!=""&&$sub_total_hrs>0){
...
<div class="col-lg-12 dashboard-stat2 bordered">
    <form method="get" action="<?=base_url()?>admin/task/statistics">
        <input type="hidden" name="id_project" value="<?= $id_projects?>" >
<div class="form-group">
<div class="col-md-3">
<span class="help-block"><?= lang("fmdate");?></span>
<input name="startdate"  value="<?=date("Y-m-d H:i");?>"  style="direction: ltr;width: 100%;" size="18" class="form-control" type="date" required>
</div>

<div class="col-md-3">
<span class="help-block"><?= lang("todate");?></span>
<input name="enddate"  value="<?=date("Y-m-d H:i");?>"  style="direction: ltr;width: 100%;" size="18" class="form-control" type="date" required>
</div>
<div class="col-md-2">
    <span class="help-block"> <br> </span>
<button id="sample_editable_1_2_new" class="btn sbold green"><?= lang("search");?><i class="fa fa-search"></i></button>
</div>
...
if($start_date!=""&&$select_date==2&&$finished_date!=""&&$select_enddate==2&&(strtotime($finished_date)>=strtotime($current_date))){
										...
										$reaming_time=ceil((strtotime($finished_date)-strtotime($current_date))/(60*60*24)).lang("day");
										 }
										 $reaming_value=1;
...
<a  href="<?=DIR?>admin/task/change_status?id_project=<?=$project_id;?>&id_status=<?php echo $data->id;?>" title=" <?= lang("status");?>" style="padding: 1px 0px;">
	...
	<?php 	if($this->session->userdata('alltasks_user')=="alltasks_user"){ ?>
				<td> <a href="<?= base_url()?>admin/task/tasks_user?user_id=<?= $user_id; ?>&id_project=<?= $project_id; ?>"><?=$user_main_name;?></a></td>
												<?php } else {?>
		<td> <?=$user_main_name;?></td>										
								...
								<div class="row">
								    ...
								    <div class="col-md-12" style="margin-top:20px">
												...
												<!--	<button id="sample_editable_1_2_new" class="btn sbold GREY button_new"><?= lang("task_button_new");?> 
													</button>-->
														<button id="sample_editable_1_2_new" class="btn sbold GREY button_other"><?= lang("task_button_other");?> 
													</button>
												<!--	<button id="sample_editable_1_2_new" class="btn sbold GREY button_note"><?= lang("task_button_note");?> 
													</button>-->
...
$(document).ready(function(e) {
	...
	$(".waitbutton").click(function(e) {
        ...
        window.location.assign("<?= DIR?>admin/task/monthly_tasks?id_project="+<?=$this->input->get('id_project');?>);
    });
        ...
        window.location.assign("<?= DIR?>admin/task/tasks_type?type=3&id_project="+<?=$this->input->get('id_project');?>);
    });
    	$(".button_new").click(function(e) {
        window.location.assign("<?= DIR?>admin/task/tasks_type?type=1&id_project="+<?=$this->input->get('id_project');?>);
    });
    	$(".type_current").click(function(e) {
        window.location.assign("<?= DIR?>admin/task/tasks_type?type=1&id_project="+<?=$this->input->get('id_project');?>);
    });
     $(".button_other").click(function(e) {
        window.location.assign("<?= DIR?>admin/task/tasks_type?type=2&id_project="+<?=$this->input->get('id_project');?>);
    });
