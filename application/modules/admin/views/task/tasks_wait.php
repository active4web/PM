...
<?php }?>
					...
					<div class="row">
						...
						$deps_total_hrs=explode(",",$deps_total_hrs);
					
						
if(count($deps_total_hrs)>0){
$main_arr=array();
for($j=0; $j<count($deps_total_hrs); $j++){
if ($deps_total_hrs[$j]!=0) {
array_push($main_arr,$deps_total_hrs[$j]);
}
}
}
$reaming_total_dep="";
...
?>
<div class="row" style="<?php if(count($type_id)>2){?>margin-left:-18px;margin-right:-20px;<?php } else {?>margin:0px<?php }?>">
					<?php
					...
					for($i=0; $i<count($type_id); $i++){
					    ...
					    $dep_progress=ceil(get_sub("user_task_log","total_hrs",array("id_project"=>$id_projects,'id_service'=>$service_id,'status'=>'2'))/60);
					    if(count($main_arr)>0){ $reaming_total_dep=ceil($main_arr[$i]-$dep_progress);}
                           ?>
...
<div class="row"  style="<?php if(count($type_id)>2){?>margin-left:-18px;margin-right:-20px;<?php } else {?>margin:0px<?php }?>">
					...
					for($i=0; $i<count($type_id); $i++){
					    $service_name=get_table_filed("services_type",array("id"=>$type_id[$i]),"name");
					      $service_id=get_table_filed("services_type",array("id"=>$type_id[$i]),"id");
					    $dep_progress=ceil(get_sub("user_task_log","total_hrs",array("id_project"=>$id_projects,'id_user'=>$id_admin,'id_service'=>$service_id,'status'=>'2'))/60);
					     $reaming_total_dep=ceil($main_arr[$i]-$dep_progress);
                           ?>
							<div class="col-md-<?php if(count($type_id)>2){?>4<?php } else {?>6<?php }?> col-xs-12" style="float:left;direction: ltr; padding-right:2px;padding-left:2px;">
							    <span><i class="fa fa-external-link" style="margin-right:0px;"></i></span>
							    <span style="font-size:10px;color: #2276bf;font-weight: bold;"><?= $service_name;?></span>
						...
						<div class="row" style="margin:0px;">
							...
							<h3 style="font-size:13px;color: #e7505a;">
							    <span><i class="fa fa-clock-o" style="margin-right:5px;"></i></span><?= lang("project duration total");?>:
							     ...
							     $reaming_total=ceil($total_hrs_project-$sub_total_hrs);
							     ?>
