...
$array=array('user_id'=>$id_worker,'project_id'=>$id_project,'status'=>'2');
$count_task=$this->db->get_where("tbl_tasks",$array)->result();
$p_total_hrs="";
$query = $this->db->query("select sum(total_hrs) as total_hrs from user_task_log where id_user=$id_worker and id_project=$id_project and status='2'");
foreach ($query->result() as $row){$p_total_hrs= round(($row->total_hrs)/60,2); }

$over_time_hrs="";
$query1 = $this->db->query("select sum(over_time_hrs) as over_time_hrs from tbl_tasks where user_id=$id_worker and project_id=$id_project and status='2'");
...
<div class="col-lg-12 dashboard-stat2 bordered">
    <form method="get" action="<?=base_url()?>admin/task/fliter">
...
<button id="sample_editable_1_2_new" class="btn sbold green addbutton"><?= lang("search");?><i class="fa fa-search"></i></button>
</div>
...
$(document).ready(function(e) {
	...
	$(".my_monthly_tasks").click(function(e) {
        window.location.assign("<?= DIR?>admin/task/user_monthly_tasks?user_id="+<?=$this->input->get('user_id');?>+"&id_project="+<?=$this->input->get('id_project');?>);
    });
        ...
        window.location.assign("<?= DIR?>admin/task/user_tasks_type?user_id="+<?=$this->input->get('user_id');?>+"&type=3&id_project="+<?=$this->input->get('id_project');?>);
    });
